...
class Clips(object):
    ...
    def __init__(self, world, Osizes):
        ...
        self.rect_view_w = (
            self.size[0] / self.a) - ((self.size[0] / self.a) / 5)
    ...
    def render(self, surface, tp, mouse_pos):
        ...
        pygame.draw.rect(
            surface, (255, 255, 255), (0, self.size[1] - 300, self.size[0], 300))
        surface.blit(
            self.minimap,
            (self.size[0] -
             self.minimap_size[0],
        ...
        pygame.draw.rect(surface, (0, 0, 0), (
            self.size[0] - self.minimap_size[0], self.size[1] -
            self.minimap_size[1], self.minimap_size[0],
            self.minimap_size[1]), 2)
...
class sidebar(object):
    ...
    def __init__(self, world, size):
        ...
        for x in range(self.len_tiles_x):
            ...
            for y in range(self.len_tiles_y):
                try:
                    self.tiles[x].append(
                        icon_tile(self.tile_size, buildings[num], (x * self.tile_width, y * self.tile_height)))
                    self.tiles[x][y].render(self.top_render)
                    pygame.draw.rect(self.top_render, self.outline,
                                     (x * self.tile_width, y * self.tile_height, self.tile_width, self.tile_height), 2)
                except Exception:
    ...
    def render(self, surface, mouse_pos, tp):
        ...
        population = "POPULATION: %d/%d" % (
            self.world.population, self.world.MAXpopulation)
        ...
        rendered_population = self.world.font.render(
            population, True, (255, 255, 255))
        rendered_pos_str = self.world.font.render(
            pos_str, True, (255, 255, 255))

        pygame.draw.rect(surface, self.outline, self.bottom_rect, 4)

        surface.blit(
            rendered_height,
            (self.bottom_rect.topleft[0] +
             5,
             self.bottom_rect.topleft[1] +
             15))
        surface.blit(
            rendered_name,
            (self.bottom_rect.topleft[0] +
...
class icon_tile(object):
    ...
    def __init__(self, const, representing, pos):
        self.selected_image = pygame.image.load(
            "Images/Buildings/SELECTED_%s_Icon.png" %
            representing)
        self.icon_image = pygame.image.load(
            "Images/Buildings/%s_Icon.png" %
            representing)
